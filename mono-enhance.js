...
(function () {
  ...
  /* ☎️ IL TELEFONO E WHATSAPP (15/8/2026)
     Il numero arriva da window.MONO_TELEFONO (mono-config.js), che e' il
     PUNTO UNICO: qui non c'e' nessun numero scritto a mano.
     ⚠️ Se il punto unico e' vuoto la funzione esce e la scheda resta col
     suo testo: meglio muta che con un numero sbagliato.
     ⚠️ E' un <p class>, NON uno <span>: dentro .page-card uno <span>
     sciolto diventa l'occhiello in alto (trappola pagata il 28/7). */
  var ICONA_WHATSAPP =
    '<svg class="mono-whatsapp__segno" viewBox="0 0 24 24" aria-hidden="true" focusable="false">' +
  ...
  function initTelefono() {
    ...
    var scheda = document.querySelector("[data-mono-telefono-card]");
    if (!scheda || !cfg.daChiamare || scheda.querySelector(".mono-recapito")) return;

    var riga = document.createElement("p");
    riga.className = "mono-recapito";

    var chiama = document.createElement("a");
    chiama.className = "mono-recapito__numero";
    chiama.href = "tel:" + cfg.daChiamare;
    chiama.textContent = cfg.daLeggere || cfg.daChiamare;
    chiama.dataset.track = "contacts_phone_call";
    riga.appendChild(chiama);

    /* WhatsApp e' facoltativo: se un domani non e' piu' attivo su quel
       numero basta svuotare `whatsapp` nel config e il tasto sparisce. */
    if (cfg.whatsapp) {
      var wa = document.createElement("a");
      wa.className = "mono-whatsapp";
      /* ⚠️ wa.me vuole il numero SENZA "+" e senza spazi: col + non apre. */
      wa.href = "https://wa.me/" + cfg.whatsapp;
      wa.target = "_blank";
      wa.rel = "noopener";
      wa.dataset.track = "contacts_whatsapp";
      wa.setAttribute("aria-label", "Scrivici su WhatsApp");
      wa.innerHTML = ICONA_WHATSAPP + "<span>WhatsApp</span>";
      riga.appendChild(wa);
    }
